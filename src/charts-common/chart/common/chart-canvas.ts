...
export abstract class ChartCanvas {
  ...
  abstract drawRRect(bounds: Rectangle, {
    fill,
    stroke,
    patternColor,
    fillPattern,
  ...
  // platform) exceeding the draw area will apply a gradient to transparent
  // with anything exceeding the x pixels to be transparent.
  abstract drawBarStack(
    canvasBarStack: CanvasBarStack,
    { drawAreaBounds }: { drawAreaBounds?: Rectangle },
  ): void;
  ...
  abstract setClipBounds(clipBounds: Rectangle): void;

  // Restore
  abstract resetClipBounds(): void;
}

export const getAnimatedColor = (previous: Color, target: Color, animationPercent: number) => {
  const r = Math.round(((target.r - previous.r) * animationPercent) + previous.r);
  const g = Math.round(((target.g - previous.g) * animationPercent) + previous.g);
  const b = Math.round(((target.b - previous.b) * animationPercent) + previous.b);
  const a = Math.round(((target.a - previous.a) * animationPercent) + previous.a);
